...
class Utils:
    @staticmethod
    def wait_such_element(driver: WebDriver, by: By, value: str, interval: int = 1,
                          limit: int = -1) -> WebElement | None:
        attempt = 0
        while True:
            try:
                if attempt == limit:
    ...
    def parse_yandex_lavka(url: str, cookie_path: str) -> dict:
        ...
        info = Utils.wait_such_element(driver, By.CSS_SELECTOR,
                                       "#root > div.hjhf26i > header > div:nth-child(6) > button")
        info.click()

        tab = Utils.wait_such_element(driver, By.CLASS_NAME, 'cnw6bwb')
